...
## Select students [userPrincipalName] only active and without AzureAd id
$mySQL= "SELECT userPrincipalName,idautoID,employeeId,givenName,surname,jobTitle,companyName,department,officeLocation,
'Hps' + right(CONVERT(NVARCHAR, dateOfBirth, 112),4) + left(CONVERT(NVARCHAR, dateOfBirth, 112),4) PSW FROM $tableSchema.$tableName WHERE accountEnabled=1 AND id is NULL"
        ...
        if (-not ([string]::IsNullOrEmpty($response))) { 
            ...
            if ($response.id -ne $null ) {
                ...
                Write-Host $response.userPrincipalName " created."  
                  ...
                  #Add email Auhtentication Method                    
                        ...
                        $UpdateUserSetting = ConvertTo-Json -InputObject $UpdateUserSetting
                         ...
                         try { 
                             $ExecuteUpdateUserSetting = Invoke-RestMethod -Uri "https://graph.microsoft.com/beta/users/$($dbUser.userPrincipalName)/authentication/emailMethods" -Method "Post" -Headers $headers -Body $UpdateUserSetting -ErrorAction Stop -UseBasicParsing
                             } catch {
                                    Write-Host "..."
                                    Write-Host "error post ExecuteUpdateUserSetting: " $dbUser.userPrincipalName                      
